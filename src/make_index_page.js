...
function GetHeadAndBodyOfHTML(html_str) {
    ...
    if (!(headstart && headend && bodystart && bodyend)) {
        throw SyntaxError("Missing body or head, or invalid syntax in advanced.html.");
    }
    headstr = pagestr.substring(headstart, headend);
    bodystr = pagestr.substring(bodystart, bodyend);
    return { head_str: headstr, head_start_index: headstart, head_end_index: headend, body_str: bodystr, body_start_index: bodystart, body_end_index: bodyend };
}
  
try {  
    let advpage = fs.readFileSync('./src/html/advanced.html', 'utf8').replace(EOLMatcher, "\n");
    ...
    advpage = advpage.replace("<title>Fairy-Stockfish playground</title>", "<title>Play against Fairy-Stockfish</title>")
        .replace(/let( +)advanced_time_control( *)=( *)false/, "let advanced_time_control = true")
