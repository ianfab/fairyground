// Simple Clicker Game - Demonstrates the standard structure
...
function initGameClient(container, socket, roomId, emitAction) {
  ...
  container.innerHTML = `
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; gap: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
      ...
      <div style="color: rgba(255,255,255,0.6); font-size: 14px;">Click to increase the score!</div>
    </div>
  `;
  ...
  // Return game instance with required onStateUpdate method
  return {
    onStateUpdate: (state) => {
      ...
      if (state.players && Object.keys(state.players).length > 0) {
        ...
        playersEl.innerHTML = 'Waiting for players...';
      }
    }
  };
}
...
// ============================================
const serverLogic = {
  // Initial state when room is created
  initialState: {
    score: 0,
    players: {},
    totalClicks: 0,
  ...
  moves: {
    ...
    click: (state, payload, playerId) => {
      // Initialize player if this is their first click
      ...
      state.totalClicks += 1;
      state.lastClickTime = payload.timestamp;
    },
    
    // Optional: Reset game
    reset: (state, payload, playerId) => {
      state.score = 0;
      state.totalClicks = 0;
      // Keep players but reset their clicks
      Object.keys(state.players).forEach(id => {
        state.players[id].clicks = 0;
      });
    }
  }
};
