...
## Colyseus Server (Alternative for 3D Games)

For more complex 3D chess variant games, a Colyseus-based server is available:

```
┌─────────────────────────────────────────────────────────────┐
│  Colyseus Server (localhost:2567)                          │
│  - Schema-based state synchronization                       │
│  - Built-in matchmaking                                     │
│  - Automatic reconnection handling                          │
│  - Web-based monitoring dashboard                           │
│  - Optimized for 3D game experiences                        │
└─────────────────────────────────────────────────────────────┘
```

### When to Use Colyseus vs Socket.io

**Use Socket.io Server** (apps/server) for:
- Simple 2D games
- Games generated by AI
- Quick prototyping
- VM-sandboxed game logic

**Use Colyseus Server** (apps/colyseus-server) for:
- 3D chess variant games
- Games requiring advanced state synchronization
- Production deployments with scaling needs
- Games with complex room management
- Built-in matchmaking requirements

See `apps/colyseus-server/README.md` for deployment instructions.
